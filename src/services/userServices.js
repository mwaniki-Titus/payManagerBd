...
export const updateEmployeeService = async (employee) => {
    const { EmployeeID, FirstName, LastName, Location, BirthDate, Contact, Gender, admin, PositionID,
    ...
    try {
        ...
        const result = await poolRequest()
            ...
            .query(
                `UPDATE Employees 
                SET FirstName = @FirstName, LastName = @LastName, Location = @Location, BirthDate = @BirthDate, 
                Contact = @Contact, Gender = @Gender, admin = @admin, PositionID = @PositionID, ScheduleID = @ScheduleID, 
                PhotoURL = @PhotoURL, Email = @Email, Password = @Password, BankName = @BankName, BankBranch = @BankBranch, 
                AccountNumber = @AccountNumber, Bio = @Bio 
